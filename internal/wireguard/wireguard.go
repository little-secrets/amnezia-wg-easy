...
func (wg *WireGuard) generateNewConfig() (*models.WGConfig, error) {
	...
	return &models.WGConfig{
		Server: models.ServerConfig{
			...
			Address:    address,
			Jc:         strconv.Itoa(wg.cfg.Jc),
			Jmin:       strconv.Itoa(wg.cfg.Jmin),
			Jmax:       strconv.Itoa(wg.cfg.Jmax),
			S1:         strconv.Itoa(wg.cfg.S1),
			S2:         strconv.Itoa(wg.cfg.S2),
			H1:         strconv.FormatUint(uint64(wg.cfg.H1), 10),
			H2:         strconv.FormatUint(uint64(wg.cfg.H2), 10),
			H3:         strconv.FormatUint(uint64(wg.cfg.H3), 10),
			H4:         strconv.FormatUint(uint64(wg.cfg.H4), 10),
		},
